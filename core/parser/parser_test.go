...
var (
	...
	MULTIPLEResponses = append(SINGLEResponse, append(SINGLEResponseMulti, []byte{10}...)...)
...
func test(response *Response.Response, testAgainst *Response.Response, identifier string, t *testing.T) {
	...
	if response.IsMultiLine() != testAgainst.IsMultiLine() {
		t.Fatal("Invalid " + identifier + " response parsed message", response.IsMultiLine())
	}
}
...
func TestMultipleMessages(t *testing.T) {
	parser := NewParser()
	parser.ParseBlock(MULTIPLEResponses)
